...
/// [ProductSearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [ProductSearchDelegate] can only be associated with one active [showSearch]
/// call. Call [ProductSearchDelegate.close] before re-using the same delegate instance
...
/// The animation provided by [ProductSearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [ProductSearchDelegate] to define the content of the search page.
Future<T> showCustomSearch<T>({
  @required BuildContext context,
  @required ProductSearchDelegate<T> delegate,
  String query = '',
}) {
...
/// query text field can be customized via [ProductSearchDelegate.buildLeading]
/// and [ProductSearchDelegate.buildActions]. Additonally, a widget can be placed
/// across the bottom of the [AppBar] via [ProductSearchDelegate.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [ProductSearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [ProductSearchDelegate.buildResults].
///
/// [ProductSearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [ProductSearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [ProductSearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [ProductSearchDelegate.close] should be
...
/// ## Handling emojis and other complex characters
/// {@macro flutter.widgets.EditableText.onChanged}
abstract class ProductSearchDelegate<T> {
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [ProductSearchDelegate.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [ProductSearchDelegate.buildResults].
  results,
}
...
class SearchPageRoute<T> extends PageRoute<T> {
  ...
  final ProductSearchDelegate<T> delegate;
